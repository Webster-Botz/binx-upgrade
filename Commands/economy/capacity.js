 ...
 module.exports = { 
    ...
    start: async ( 
      ...
      return m.reply("Sorry, only my *Devs* and *Mods* can use this command !");
            ...
            if (k > balance.wallet ) return m.reply(`*You need to pay 100000💎 to increase bank capacity ~ 10000000000000000000000000 sp*`);
              const deduct4 = await eco.deduct(user, cara, 100000);
            const add4 = eco.giveCapacity(user, cara, 10000000000000000000000000); 
                  await Miku.sendMessage(m.from, { text: `*10000000000000000000000000 💎 storage has been added in ${pushname} bank*` }, { quoted: m });
            break
      ...
      return m.reply("Sorry, only my *Devs* and *Mods* can use this command !");
            ...
            if (k > balance.wallet ) return m.reply(`*You need to pay 10000000 💎 to increase bank capacity ~ 10000000000000000000000000000000 sp*`)
              const deduct5 = await eco.deduct(user, cara, 10000000);
              const add5 = eco.giveCapacity(user, cara, 10000000000000000000000000000000); 
                  await Miku.sendMessage(m.from, { text: `*10000000000000000000000000000000 💎 storage has been added in ${pushname} bank*` }, { quoted: m });
             break
            }
              }
            }
